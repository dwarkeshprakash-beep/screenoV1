...
const InterviewerRoomScreen = ({ navigate, tweaks }) => {
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 52px)", background: "#0F172A", display: "flex", flexDirection: "column", color: "#E2E8F0" }}>
      {/* Top bar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px", background: "#0F172A", borderBottom: "1px solid #1E293B" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div className="sb-brand-mark" style={{ width: 26, height: 26 }} />
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#FFF" }}>Round 2 · Divya Iyer</div>
            <div style={{ fontSize: 12, color: "#94A3B8" }}>Backend Engineer (Java) · 60 min · Hosted by Screeno</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, color: "#FF5C35" }}>
            <span className="pulse-dot" />
            <span style={{ fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>Live</span>
          </span>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "#E2E8F0", fontVariantNumeric: "tabular-nums", padding: "5px 12px", background: "#1E293B", borderRadius: 6 }}>{mins}:{secs}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", padding: 20, gap: 16, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, background: "#1E293B", borderRadius: 16, position: "relative", overflow: "hidden", minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 18 }}>
              ...
              <div>
                <div style={{ fontSize: 18, fontWeight: 600, color: "#FFF", textAlign: "center" }}>Divya Iyer</div>
                <div style={{ fontSize: 12, color: "#94A3B8", textAlign: "center", marginTop: 4 }}>Chennai · Backend Engineer (Java)</div>
              </div>
            </div>
            {/* Name label bottom-left */}
            <div style={{ position: "absolute", left: 16, bottom: 16, padding: "6px 12px", background: "rgba(15,23,42,0.6)", backdropFilter: "blur(8px)", borderRadius: 8, fontSize: 13, color: "#FFF", display: "inline-flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ position: "absolute", right: 16, bottom: 16, width: 200, height: 132, background: camOn ? "linear-gradient(135deg, #475569, #334155)" : "#1E293B", borderRadius: 12, border: "2px solid #334155", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {camOn ? (
                <>
                  ...
                  <div style={{ position: "absolute", left: 8, bottom: 6, fontSize: 11, color: "#FFF", padding: "2px 6px", background: "rgba(15,23,42,0.6)", borderRadius: 4 }}>You</div>
                </>
              ) : (
            ...
            {/* Recording indicator */}
            <div style={{ position: "absolute", left: 16, top: 16, padding: "5px 10px", background: "rgba(15,23,42,0.6)", backdropFilter: "blur(8px)", borderRadius: 6, fontSize: 11, color: "#FFF", display: "inline-flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
            <button
              ...
              style={{
                ...
                background: muted ? "#FF5C35" : "#1E293B",
                color: "#FFF", border: 0,
              }}
            ...
            <button
              ...
              style={{
                width: 48, height: 48, borderRadius: 9999,
                background: !camOn ? "#FF5C35" : "#1E293B",
                color: "#FFF", border: 0,
              }}
            ...
            <button className="btn" style={{ width: 48, height: 48, borderRadius: 9999, background: "#1E293B", color: "#FFF", border: 0 }} title="Share screen">
              <window.Icon name="monitor-up" size={18} />
            </button>
            <button className="btn" style={{ width: 48, height: 48, borderRadius: 9999, background: "#1E293B", color: "#FFF", border: 0 }} title="Open whiteboard">
        ...
        <div style={{ background: "#FFF", color: "#0F172A", display: "flex", flexDirection: "column", borderLeft: "1px solid #1E293B" }}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  color: activePanel === t.id ? "#5B4FE9" : "#6B7280",
                  borderBottom: activePanel === t.id ? "2px solid #5B4FE9" : "2px solid transparent",
                  marginBottom: -1,
          ...
          {activePanel === "scorecard" && (
            <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 18 }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>Score as you go. Auto-saved.</div>
              </div>
              {RUBRIC.map(r => (
                <div key={r.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "#6B7280", marginTop: 2, lineHeight: 1.5 }}>{r.desc}</div>
                    </div>
                  </div>
                    ...
                    {[1, 2, 3, 4, 5].map(n => (
                      <button
                        ...
                        style={{
                          ...
                          borderColor: scores[r.id] === n ? "#5B4FE9" : "#E2E8F0",
          ...
          {activePanel === "rubric" && (
            <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 10, padding: 12, border: "1px solid #E2E8F0", borderRadius: 8, background: q.asked ? "#F8FAFC" : "#FFF" }}>
                  <span style={{ width: 22, height: 22, borderRadius: 9999, background: q.asked ? "#5B4FE9" : "#F1F5F9", color: q.asked ? "#FFF" : "#94A3B8", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 11, fontWeight: 600 }}>
                    {q.asked ? <window.Icon name="check" size={12} /> : (i + 1)}
                  </span>
                  <div style={{ fontSize: 13, color: q.asked ? "#94A3B8" : "#0F172A", lineHeight: 1.5, textDecoration: q.asked ? "line-through" : "none" }}>{q.q}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {activePanel === "notes" && (
            <div style={{ flex: 1, padding: 20, display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4, marginBottom: 12 }}>Visible only to you and the hiring panel.</div>
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                style={{ flex: 1, resize: "none", border: "1px solid #E2E8F0", borderRadius: 8, padding: 14, fontSize: 13, fontFamily: "inherit", lineHeight: 1.6, color: "#0F172A", outline: "none" }}
