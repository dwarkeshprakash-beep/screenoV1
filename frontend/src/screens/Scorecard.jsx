...
const ScorecardScreen = ({ candidate, navigate, onSubmit }) => {
  ...
  const recommendOptions = [
    { id: "strong-hire", label: "Strong hire",   icon: "thumbs-up",      color: "#047857", bg: "#ECFDF5" },
    { id: "hire",        label: "Hire",           icon: "check-circle-2", color: "#059669", bg: "#ECFDF5" },
    { id: "no-hire",     label: "No hire",        icon: "x-circle",       color: "#B53618", bg: "#FFEDE6" },
  ...
  return (
    <div className="fadeup" style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 24, alignItems: "start" }}>
      ...
      <div className="card card-pad-lg">
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, paddingBottom: 22, borderBottom: "1px solid #F1F5F9" }}>
          <window.Avatar name={candidate.name} size={48} />
          <div style={{ flex: 1 }}>
            <window.Eyebrow>Scorecard · Round 2 · Tech</window.Eyebrow>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 600, color: "#0F172A", marginTop: 4 }}>How did {candidate.name.split(" ")[0]} perform?</div>
            <div style={{ fontSize: 13, color: "#6B7280", marginTop: 2 }}>{candidate.role} · 60 min interview</div>
          </div>
          ...
          {RUBRIC.map(r => (
            <div key={r.id}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontWeight: 600, fontSize: 18, color: scores[r.id] >= 4 ? "#047857" : scores[r.id] >= 3 ? "#B45309" : "#B53618" }}>
                  {scores[r.id] || "—"}
                </div>
              </div>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid",
                      borderColor: scores[r.id] === opt.v ? "#5B4FE9" : "#E2E8F0",
                      background: scores[r.id] === opt.v ? "#EFEDFD" : "#FFF",
                      color: scores[r.id] === opt.v ? "#3A31A3" : "#374151",
            ...
            {recommendOptions.map(opt => (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${recommend === opt.id ? opt.color : "#E2E8F0"}`,
                  background: recommend === opt.id ? opt.bg : "#FFF",
                  color: recommend === opt.id ? opt.color : "#6B7280",
                  cursor: "pointer", transition: "all 120ms",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 88 }}>
        <div className="card card-pad" style={{ textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "var(--font-display)", fontSize: 56, fontWeight: 700, color: "#0F172A", letterSpacing: "-0.03em", lineHeight: 1, marginTop: 12 }}>
            {overall}
          </div>
          <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4 }}>/ 5.0</div>
          <div style={{ height: 6, background: "#F1F5F9", borderRadius: 9999, marginTop: 16, overflow: "hidden" }}>
            ...
            {[
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#6B7280" }}>{r.round}</div>
                </div>
                <div style={{ fontFamily: "var(--font-mono)", fontWeight: 600, fontSize: 13, color: "#047857" }}>{typeof r.s === "number" ? r.s.toFixed(1) : r.s}</div>
              </div>
            ))}
          </div>
        </div>

        <div style={{ padding: 14, background: "#F8FAFC", borderRadius: 10, fontSize: 12, color: "#6B7280", lineHeight: 1.5 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#0F172A", fontWeight: 600, marginBottom: 6 }}>
            <window.Icon name="info" size={14} color="#5B4FE9" /> Scoring guidance
          </div>
