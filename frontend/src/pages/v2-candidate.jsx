  ...
  return (
    <div style={{minHeight:"calc(100vh - 132px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
      <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:16,padding:"40px 32px",maxWidth:480,width:"100%",boxShadow:"0 8px 28px rgba(15,23,42,0.06)",textAlign:"center"}}>
        ...
        <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.02em",margin:"0 0 8px"}}>Senior .NET Developer</h1>
        <p style={{fontSize:14,color:"var(--slate-500)",margin:"0 0 28px"}}>Acme Corp is inviting you to the next step.</p>
        ...
        <button onClick={onNext} style={{
          ...
          background:"linear-gradient(135deg,var(--brand-500),var(--brand-600))",color:"var(--bg-surface)",border:0,
...
const DeviceCheck = ({onNext}) => {
  ...
  return (
    <div style={{padding:"40px 24px",maxWidth:540,margin:"0 auto"}}>
      <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:24,fontWeight:700,color:"var(--slate-900)",margin:"0 0 4px",letterSpacing:"-0.02em"}}>Let's check your device</h1>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
        {checks.map((c,i)=>{
          ...
          const iconBg  = st==="passed"?"var(--success-50)":st==="checking"?"var(--brand-50)":st==="failed"?"#FEF2F2":"var(--slate-100)";
          const iconClr = st==="passed"?"var(--success-500)":st==="checking"?"var(--brand-500)":st==="failed"?"#EF4444":"var(--slate-400)";
          return (
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                {st==="passed"&&c.extraType==="level"&&(
                  <div style={{display:"flex",alignItems:"center",gap:2,height:24}}>
                    {bars.map((v,j)=><span key={j} style={{width:3,height:`${Math.max(6,v*22)}px`,background:"var(--brand-500)",borderRadius:9999,transition:"height 180ms"}}/>)}
                  </div>
                )}
                {c.id==="speaker"&&st!=="passed"&&(
                  <button onClick={()=>{setSpeakerTest(true);setStatus(s=>({...s,speaker:"passed"}));}} style={{padding:"5px 10px",border:"1px solid var(--slate-300)",borderRadius:7,background:"var(--bg-surface)",fontSize:12,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5}}>
                ...
                {st==="checking"&&<i data-lucide="loader-2" style={{width:20,height:20,color:"var(--slate-400)",animation:"v2spin 1s linear infinite"}}/>}
      ...
      <button onClick={onNext} disabled={!allPassed} style={{
        ...
        background:allPassed?"var(--brand-500)":"var(--slate-200)",color:allPassed?"var(--bg-surface)":"var(--slate-400)",
...
const ConsentScreen = ({onAgree, onDecline}) => {
  ...
  return (
    <div style={{padding:"40px 24px",maxWidth:600,margin:"0 auto"}}>
      <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.02em",margin:"0 0 6px"}}>Before you begin</h1>
      <p style={{fontSize:14,color:"var(--slate-500)",margin:"0 0 24px"}}>Please review what will be recorded during your interview.</p>
      ...
      <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:14,padding:24,marginBottom:18,boxShadow:"0 1px 3px rgba(15,23,42,0.04)"}}>
        {items.map((it,i)=>(
          <div key={it.title} style={{display:"flex",gap:14,padding:`${i===0?0:14}px 0 ${i===items.length-1?0:14}px`,borderTop:i===0?"0":"1px solid var(--slate-100)"}}>
      ...
      <label onClick={()=>setAgreed(!agreed)} style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px",background:agreed?"#F3F0FF":"var(--slate-50)",border:`1px solid ${agreed?"var(--brand-500)":"var(--slate-200)"}`,borderRadius:10,cursor:"pointer",marginBottom:14,transition:"all 160ms"}}>
        <span style={{width:20,height:20,borderRadius:5,background:agreed?"var(--brand-500)":"var(--bg-surface)",border:`1.5px solid ${agreed?"var(--brand-500)":"var(--slate-300)"}`,display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 160ms"}}>
          {agreed&&<i data-lucide="check" style={{width:13,height:13,color:"var(--bg-surface)"}}/>}
        </span>
      ...
      <button onClick={onAgree} disabled={!agreed} style={{
        ...
        background:agreed?"var(--brand-500)":"var(--slate-200)",color:agreed?"var(--bg-surface)":"var(--slate-400)",
      ...
      <div style={{textAlign:"center",marginTop:12}}>
        <button onClick={onDecline} style={{background:"transparent",border:0,color:"var(--slate-400)",fontSize:13,cursor:"pointer",textDecoration:"underline",fontFamily:"inherit"}}>
          I do not consent
        </button>
      </div>
...
const AIScreenRoom = ({onEnd}) => {
  ...
  const q = QS[qIndex];
  const orbState = {
    speaking:  {bg:"linear-gradient(135deg,var(--brand-100),var(--brand-500) 70%,var(--brand-600))", shadow:"0 12px 36px rgba(91,79,233,0.35)", label:"Asking…",   iconName:"volume-2", labelColor:"var(--brand-500)"},
    thinking:  {bg:"radial-gradient(circle at 35% 30%,var(--brand-100),var(--brand-500) 90%)", shadow:"0 12px 28px rgba(91,79,233,0.18)", label:"Processing…", iconName:"loader-2", labelColor:"var(--slate-400)"},
    listening: {bg:"radial-gradient(circle at 35% 30%,var(--success-100),var(--success-500) 75%)", shadow:"0 12px 28px rgba(5,150,105,0.32)", label:"Listening…", iconName:"ear", labelColor:"var(--success-500)"},
  }[aiState];
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 440px",minHeight:"calc(100vh - 132px)",position:"relative"}}>
      ...
      {warnVisible && (
        <div style={{position:"absolute",inset:0,background:"rgba(15,23,42,0.7)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"var(--bg-surface)",borderRadius:16,padding:"32px 28px",maxWidth:420,textAlign:"center",boxShadow:"0 24px 48px rgba(15,23,42,0.24)"}}>
            ...
            <div style={{fontFamily:"var(--font-display,'Inter')",fontSize:20,fontWeight:700,color:"var(--slate-900)",marginBottom:8}}>Tab switch detected</div>
            ...
            <button onClick={resumeAfterWarn} style={{width:"100%",padding:"12px 20px",background:"var(--brand-500)",color:"var(--bg-surface)",border:0,borderRadius:10,fontWeight:600,fontSize:14,cursor:"pointer"}}>Resume interview</button>
          </div>
        </div>
      )}
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 48px",gap:22,background:"var(--bg-surface)",borderRight:"1px solid var(--slate-200)",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:16,left:16,display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:600}}>
          <span style={{width:7,height:7,borderRadius:9999,background:phase==="recording"?"#EF4444":"var(--slate-400)",animation:phase==="recording"?"v2pulse 1.4s ease-in-out infinite":"none"}}/>
          <span style={{color:phase==="recording"?"#EF4444":"var(--slate-400)"}}>{phase==="recording"?"RECORDING":"STANDBY"}</span>
        </div>
        <div style={{position:"absolute",top:14,right:16,display:"inline-flex",alignItems:"center",gap:6,fontFamily:"monospace",fontSize:13,fontWeight:700,color:remaining<120?"#EF4444":"var(--slate-900)",background:"var(--slate-100)",padding:"5px 10px",borderRadius:8,fontVariantNumeric:"tabular-nums"}}>
        ...
        <div style={{position:"relative",width:160,height:160,display:"flex",alignItems:"center",justifyContent:"center"}}>
          {aiState==="listening" && [0,1,2].map(i=>(
            <span key={i} style={{position:"absolute",width:150,height:150,borderRadius:9999,border:"2px solid var(--success-500)",animation:`v2ring 2.4s ease-out ${i*800}ms infinite`,opacity:0}}/>
          ))}
          <div style={{width:130,height:130,borderRadius:9999,background:orbState.bg,boxShadow:orbState.shadow,display:"flex",alignItems:"center",justifyContent:"center",animation:paused?"none":"v2orbpulse 2s ease-in-out infinite",position:"relative"}}>
            ...
            <i data-lucide="sparkles" style={{width:46,height:46,color:"var(--bg-surface)",position:"relative",zIndex:1}}/>
          </div>
        </div>

        <div style={{textAlign:"center"}}>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:4}}>Screeno AI · Question {qIndex+1} of {QS.length}</div>
        ...
        <div style={{width:"100%",maxWidth:440,background:"#FAFAFE",border:"1px solid var(--brand-100)",borderRadius:14,padding:"18px 20px",textAlign:"center"}}>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:8}}>{q?.phase}</div>
          <div style={{fontSize:16,color:"var(--slate-900)",lineHeight:1.5,fontWeight:500}}>{phase==="done"?"That's the last question — thanks! You can end the interview now.":q?.text}</div>
          {phase!=="done" && (
            <div style={{marginTop:12,fontSize:12,color:phase==="recording"&&qRemaining<15?"#EF4444":"var(--slate-400)",display:"inline-flex",alignItems:"center",gap:5,fontFamily:"monospace"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,height:40}}>
          {[...Array(16)].map((_,i)=>(
            <span key={i} style={{width:4,height:34,borderRadius:9999,background:phase==="recording"?"var(--success-500)":aiState==="speaking"?"var(--brand-500)":"var(--slate-300)",animation:paused||phase==="processing"?"none":`v2wave 1.2s ease-in-out ${i*70}ms infinite`,transform:"scaleY(0.25)",transformOrigin:"center"}}/>
          ))}
        </div>

        {/* Controls: Start/Stop record + mute + end */}
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <button onClick={()=>setMuted(m=>!m)} title="Mute" style={{width:52,height:52,borderRadius:9999,background:muted?"#FEF2F2":"var(--bg-surface)",border:`1px solid ${muted?"#FECACA":"var(--slate-300)"}`,color:muted?"#EF4444":"var(--slate-900)",display:"inline-flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 120ms"}}>
            <i data-lucide={muted?"mic-off":"mic"} style={{width:20,height:20}}/>
          </button>

          {phase==="asking" && (
            <button onClick={startAnswer} style={{height:52,padding:"0 26px",borderRadius:9999,background:"var(--success-500)",color:"var(--bg-surface)",border:0,fontSize:14,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 6px 16px rgba(5,150,105,0.3)"}}>
              <i data-lucide="circle" style={{width:16,height:16,fill:"var(--bg-surface)"}}/> Start answer
            </button>
          )}
          {phase==="recording" && (
            <button onClick={stopAnswer} style={{height:52,padding:"0 26px",borderRadius:9999,background:"#EF4444",color:"var(--bg-surface)",border:0,fontSize:14,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 6px 16px rgba(239,68,68,0.3)",animation:"v2recpulse 1.6s ease-in-out infinite"}}>
              <i data-lucide="square" style={{width:15,height:15,fill:"var(--bg-surface)"}}/> Stop answer · {clk(recElapsed)}
            </button>
          )}
          {phase==="processing" && (
          ...
          {phase==="done" && (
            <button onClick={onEnd} style={{height:52,padding:"0 26px",borderRadius:9999,background:"var(--brand-500)",color:"var(--bg-surface)",border:0,fontSize:14,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 6px 16px rgba(91,79,233,0.3)"}}>
              <i data-lucide="check" style={{width:16,height:16}}/> Finish & submit
            </button>
          )}

          <button onClick={onEnd} title="End interview" style={{width:52,height:52,borderRadius:9999,background:"var(--bg-surface)",border:"1px solid var(--danger-100)",color:"var(--danger-600)",display:"inline-flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
        ...
        <button onClick={simulateTabSwitch} style={{position:"absolute",bottom:14,right:14,padding:"5px 10px",background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:7,fontSize:11,color:"var(--slate-400)",cursor:"pointer"}}>Simulate tab switch</button>
        <div style={{position:"absolute",bottom:14,left:14,width:84,height:84,borderRadius:9999,background:"linear-gradient(135deg,#475569,#1E293B)",border:"3px solid var(--bg-surface)",boxShadow:"0 8px 20px rgba(15,23,42,0.2)",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",background:"var(--bg-surface)"}}>
        <div style={{padding:"16px 24px",borderBottom:"1px solid var(--slate-100)"}}>
          ...
          <div style={{fontSize:12,color:"var(--slate-400)",marginTop:2}}>Builds as the interview goes · upcoming questions stay hidden</div>
        </div>
            ...
            <div key={i} style={{display:"flex",gap:12}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                  <span style={{fontSize:12,fontWeight:600,color:t.who==="ai"?"var(--brand-500)":"var(--slate-900)"}}>{t.who==="ai"?"Screeno AI":"Rahul"}</span>
                  <span style={{fontFamily:"monospace",fontSize:11,color:"var(--slate-400)"}}>{t.t}</span>
                </div>
                <div style={{fontSize:14,color:"var(--slate-700)",lineHeight:1.65}}>{t.text}</div>
              </div>
            </div>
          ))}
          {phase==="recording" && (
            <div style={{display:"flex",gap:12,opacity:0.7}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:4,padding:"8px 0"}}>
                  {[0,1,2].map(i=><span key={i} style={{width:6,height:6,borderRadius:9999,background:"var(--slate-400)",animation:`v2pulsedot 1.4s ease-in-out ${i*0.2}s infinite`}}/>)}
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{padding:"14px 24px",borderTop:"1px solid var(--slate-100)",fontSize:12,color:"var(--slate-500)",display:"flex",alignItems:"center",gap:6}}>
          <i data-lucide="info" style={{width:13,height:13,color:"var(--slate-400)"}}/>
  ...
  return (
    <div style={{maxWidth:680,margin:"0 auto",padding:"48px 24px"}}>
      <div style={{marginBottom:28}}>
        <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)"}}>Acme Technologies</div>
        <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:36,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.025em",margin:"8px 0 6px"}}>Welcome back, Rahul.</h1>
        <p style={{fontSize:15,color:"var(--slate-700)",lineHeight:1.6}}>Your AI voice screen is scheduled for <strong style={{color:"var(--slate-900)"}}>today at 4:30 PM IST</strong>. It'll take about 25 minutes.</p>
      </div>

      {/* Next action */}
      <div style={{background:"var(--bg-surface)",border:"1px solid var(--brand-100)",borderRadius:14,padding:24,marginBottom:20,display:"flex",alignItems:"center",gap:20,boxShadow:"0 8px 24px rgba(91,79,233,0.06)"}}>
        <div style={{width:64,height:64,borderRadius:14,background:"linear-gradient(135deg,var(--brand-500),var(--brand-600))",display:"inline-flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 20px rgba(91,79,233,0.3)",flexShrink:0}}>
          <i data-lucide="mic" style={{width:28,height:28,color:"var(--bg-surface)"}}/>
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:4}}>Next up</div>
          <div style={{fontFamily:"var(--font-display,'Inter')",fontSize:20,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.01em"}}>AI voice interview</div>
          <div style={{fontSize:13,color:"var(--slate-500)",marginTop:4,display:"flex",gap:14,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,flexShrink:0}}>
          <button onClick={()=>onNavigate("c-device")} style={{padding:"11px 20px",borderRadius:10,background:"var(--brand-500)",color:"var(--bg-surface)",border:0,fontWeight:600,fontSize:14,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6,boxShadow:"0 6px 14px rgba(91,79,233,0.25)"}}>
            Start now <i data-lucide="arrow-right" style={{width:14,height:14}}/>
          </button>
          <button style={{background:"transparent",border:0,color:"var(--slate-400)",fontSize:12,cursor:"pointer"}}>Reschedule</button>
        </div>
      </div>

      {/* Journey */}
      <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(15,23,42,0.04)"}}>
        <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:16}}>Your hiring journey</div>
          ...
          {steps.map((s,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 0",position:"relative"}}>
              <span style={{
                ...
                background:s.state==="done"?"var(--brand-500)":s.state==="active"?"var(--bg-surface)":"var(--bg-surface)",
                border:s.state==="active"?"2px solid var(--brand-500)":s.state==="upcoming"?"1px solid var(--slate-200)":"0",
                boxShadow:s.state==="active"?"0 0 0 4px rgba(91,79,233,0.12)":"none",
                color:s.state==="done"?"var(--bg-surface)":s.state==="active"?"var(--brand-500)":"var(--slate-400)",
                display:"inline-flex",alignItems:"center",justifyContent:"center",
                fontSize:11,fontWeight:700,
              }}>
                {s.state==="done"?<i data-lucide="check" style={{width:13,height:13}}/>:
                 s.state==="active"?<span style={{width:8,height:8,borderRadius:9999,background:"var(--brand-500)"}}/>:
                 <span style={{fontFamily:"monospace",fontSize:11}}>{i+1}</span>}
              </span>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:s.state==="active"?600:500,color:s.state==="upcoming"?"var(--slate-400)":"var(--slate-900)"}}>{s.label}</div>
                <div style={{fontSize:12,color:s.state==="active"?"var(--brand-500)":"var(--slate-500)",marginTop:1}}>{s.when}</div>
              </div>
              {s.state==="active"&&<span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:9999,background:"var(--brand-50)",color:"var(--brand-500)"}}>In progress</span>}
              {s.state==="done"&&<span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:9999,background:"var(--success-50)",color:"var(--success-600)"}}>Done</span>}
            </div>
          ))}
        </div>
      </div>
...
const CompletionScreen = ({}) => {
  ...
  return (
    <div style={{maxWidth:640,margin:"0 auto",padding:"60px 24px",textAlign:"center"}}>
      <div style={{width:80,height:80,borderRadius:9999,background:"var(--success-100)",display:"inline-flex",alignItems:"center",justifyContent:"center",boxShadow:"0 12px 28px rgba(5,150,105,0.18),0 0 0 8px rgba(16,185,129,0.08)",marginBottom:24,animation:"v2pop 400ms cubic-bezier(0.34,1.56,0.64,1)"}}>
      ...
      <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:32,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.025em",margin:"0 0 10px"}}>You're all done!</h1>
      ...
      <div style={{background:"var(--warning-50)",border:"1px solid var(--warning-100)",borderRadius:12,padding:"16px 20px",marginBottom:28,textAlign:"left"}}>
        ...
        <div style={{fontSize:13,color:"var(--warning-700)",lineHeight:1.6}}>Strong technical depth on .NET! Consider practising system design at scale — a weak spot was identified in decomposition strategies. We've sent a detailed improvement guide to your email.</div>
      </div>

      <div style={{height:1,background:"var(--slate-200)",margin:"0 0 28px"}}/>
      <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:16}}>What happens next</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,textAlign:"left",marginBottom:28}}>
        {nexts.map((n,i)=>(
          <div key={i} style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:18,boxShadow:"0 1px 3px rgba(15,23,42,0.04)"}}>
            ...
            <div style={{fontSize:13,fontWeight:600,color:"var(--slate-900)",marginBottom:4}}>{n.title}</div>
            <div style={{fontSize:12,color:"var(--slate-500)",lineHeight:1.55}}>{n.body}</div>
          </div>
        ))}
      </div>
      <button style={{padding:"11px 20px",border:"1px solid var(--slate-300)",borderRadius:10,background:"var(--bg-surface)",fontSize:14,fontWeight:600,color:"var(--slate-700)",cursor:"pointer",transition:"all 120ms"}} onMouseEnter={e=>e.currentTarget.style.background="var(--slate-100)"} onMouseLeave={e=>e.currentTarget.style.background="var(--bg-surface)"}>Close window</button>
  ...
  return (
    <div style={{minHeight:"calc(100vh - 132px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
      <div style={{maxWidth:460,width:"100%",textAlign:"center"}}>
        <div style={{width:72,height:72,borderRadius:18,background:"linear-gradient(135deg,var(--brand-500),var(--brand-600))",display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:22,boxShadow:"0 12px 28px rgba(91,79,233,0.3)",animation:"v2orbpulse 2s ease-in-out infinite"}}>
          <i data-lucide="sparkles" style={{width:32,height:32,color:"var(--bg-surface)"}}/>
        </div>
        <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:24,fontWeight:700,color:"var(--slate-900)",letterSpacing:"-0.02em",margin:"0 0 6px"}}>Preparing your {target==="exam"?"exam":"interview"}</h1>
        <p style={{fontSize:14,color:"var(--slate-500)",margin:"0 0 26px"}}>Screeno AI is generating questions tailored to this role. This takes a few seconds.</p>

        <div style={{height:4,background:"var(--brand-50)",borderRadius:9999,overflow:"hidden",marginBottom:22}}>
          <div style={{height:"100%",width:`${pct}%`,background:"var(--brand-500)",borderRadius:9999,transition:"width 600ms cubic-bezier(0.2,0,0,1)"}}/>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,textAlign:"left"}}>
          {steps.map((s,i)=>{
            ...
            return (
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"var(--bg-surface)",border:`1px solid ${cur?"var(--brand-100)":"var(--slate-200)"}`,borderRadius:10,opacity:i>active?0.5:1,transition:"all 200ms"}}>
                <div style={{width:34,height:34,borderRadius:9,background:done?"var(--success-50)":cur?"var(--brand-50)":"var(--slate-100)",color:done?"var(--success-500)":cur?"var(--brand-500)":"var(--slate-400)",display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                ...
                <span style={{fontSize:14,fontWeight:cur?600:500,color:i>active?"var(--slate-400)":"var(--slate-900)"}}>{s.label}</span>
              </div>
            );
          })}
        </div>
        <p style={{fontSize:12,color:"var(--slate-400)",marginTop:20,display:"inline-flex",alignItems:"center",gap:6}}>
...
const ExamRunner = ({onSubmit}) => {
  ...
  if(submitted){
    ...
    return (
      <div style={{maxWidth:560,margin:"0 auto",padding:"60px 24px",textAlign:"center"}}>
        <div style={{width:72,height:72,borderRadius:9999,background:"var(--success-100)",display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:20}}>
          <i data-lucide="check-circle-2" style={{width:34,height:34,color:"var(--success-500)"}}/>
        </div>
        <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"var(--slate-900)",margin:"0 0 8px"}}>Exam submitted</h1>
        <p style={{fontSize:14,color:"var(--slate-500)",margin:"0 0 24px"}}>Your answers were recorded. Results will appear in your report.</p>
        <div style={{display:"flex",gap:14,justifyContent:"center",marginBottom:28}}>
          <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:"16px 24px"}}>
            <div style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"var(--slate-900)"}}>{correct}/{total}</div>
            <div style={{fontSize:12,color:"var(--slate-500)",marginTop:2}}>MCQ correct</div>
          </div>
          <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:"16px 24px"}}>
            <div style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"var(--slate-900)"}}>{passed}/{EXAM.coding.tests.length}</div>
            <div style={{fontSize:12,color:"var(--slate-500)",marginTop:2}}>Tests passed</div>
          </div>
        </div>
        <button onClick={onSubmit} style={{padding:"12px 26px",borderRadius:10,background:"var(--brand-500)",color:"var(--bg-surface)",border:0,fontWeight:600,fontSize:14,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8}}>
          Continue <i data-lucide="arrow-right" style={{width:14,height:14}}/>
        </button>
      </div>
    );
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:0,height:"calc(100vh - 132px)",background:"var(--slate-50)"}}>
      {/* Palette */}
      <div style={{borderRight:"1px solid var(--slate-200)",background:"var(--bg-surface)",padding:18,display:"flex",flexDirection:"column",gap:16,overflowY:"auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,fontFamily:"monospace",fontSize:16,fontWeight:700,color:remaining<300?"#EF4444":"var(--slate-900)",background:"var(--slate-100)",padding:"8px 12px",borderRadius:8,justifyContent:"center"}}>
          <i data-lucide="clock" style={{width:15,height:15}}/>{clk(remaining)}
        </div>
        <div>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--slate-400)",marginBottom:8}}>Multiple choice</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
            {EXAM.mcqs.map((m,i)=>(
              <button key={m.id} onClick={()=>setView(i)} style={{
                ...
                border:`1px solid ${view===i?"var(--brand-500)":answers[m.id]!=null?"#A7F3D0":"var(--slate-200)"}`,
                background:view===i?"var(--brand-500)":answers[m.id]!=null?"var(--success-50)":"var(--bg-surface)",
                color:view===i?"var(--bg-surface)":answers[m.id]!=null?"var(--success-600)":"var(--slate-500)",
              }}>{i+1}</button>
            ))}
          </div>
        </div>
        <div>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--slate-400)",marginBottom:8}}>Coding</div>
          <button onClick={()=>setView(total)} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"10px 12px",borderRadius:8,fontFamily:"inherit",fontSize:13,fontWeight:600,cursor:"pointer",
            border:`1px solid ${isCoding?"var(--brand-500)":"var(--slate-200)"}`,background:isCoding?"var(--brand-50)":"var(--bg-surface)",color:isCoding?"var(--brand-700)":"var(--slate-700)"}}>
            <i data-lucide="code-2" style={{width:15,height:15}}/> {EXAM.coding.title}
          </button>
        </div>
        <div style={{marginTop:"auto",fontSize:12,color:"var(--slate-500)"}}>{answeredCount}/{total} MCQs answered</div>
        <button onClick={()=>setSub(true)} style={{padding:"11px 0",borderRadius:9,background:"var(--brand-500)",color:"var(--bg-surface)",border:0,fontWeight:600,fontSize:13,cursor:"pointer"}}>Submit exam</button>
      </div>
      ...
      <div style={{overflow:"hidden",display:"flex",flexDirection:"column"}}>
        {!isCoding && (
          <div style={{padding:"32px 40px",overflowY:"auto"}}>
            <div style={{maxWidth:680}}>
              <div style={{fontSize:12,fontWeight:600,color:"var(--brand-500)",marginBottom:8}}>Question {view+1} of {total}</div>
              <h2 style={{fontSize:20,fontWeight:600,color:"var(--slate-900)",lineHeight:1.5,margin:"0 0 22px"}}>{EXAM.mcqs[view].q}</h2>
                ...
                {EXAM.mcqs[view].options.map((opt,oi)=>{
                  ...
                  return (
                    <button key={oi} onClick={()=>setAnswers(a=>({...a,[EXAM.mcqs[view].id]:oi}))} style={{
                      ...
                      border:`1px solid ${sel?"var(--brand-500)":"var(--slate-200)"}`,background:sel?"#F3F0FF":"var(--bg-surface)",transition:"all 120ms",
                    }}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginTop:28}}>
                <button onClick={()=>setView(v=>Math.max(0,v-1))} disabled={view===0} style={{padding:"10px 18px",borderRadius:8,border:"1px solid var(--slate-300)",background:"var(--bg-surface)",color:view===0?"var(--slate-300)":"var(--slate-700)",fontWeight:600,fontSize:13,cursor:view===0?"not-allowed":"pointer"}}>Previous</button>
                <button onClick={()=>setView(v=>v+1)} style={{padding:"10px 18px",borderRadius:8,border:0,background:"var(--brand-500)",color:"var(--bg-surface)",fontWeight:600,fontSize:13,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6}}>
        ...
        {isCoding && (
          <div style={{display:"grid",gridTemplateColumns:"360px 1fr",height:"100%",minHeight:0}}>
            {/* prompt */}
            <div style={{borderRight:"1px solid var(--slate-200)",background:"var(--bg-surface)",padding:"24px 22px",overflowY:"auto"}}>
              <div style={{fontSize:12,fontWeight:600,color:"var(--brand-500)",marginBottom:6}}>Coding challenge</div>
              <h2 style={{fontSize:18,fontWeight:700,color:"var(--slate-900)",margin:"0 0 12px"}}>{EXAM.coding.title}</h2>
              <p style={{fontSize:13,color:"var(--slate-700)",lineHeight:1.6,margin:"0 0 16px"}}>{EXAM.coding.prompt}</p>
              <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--slate-400)",marginBottom:8}}>Examples</div>
              {EXAM.coding.examples.map((e,i)=>(
                <div key={i} style={{background:"var(--slate-900)",borderRadius:8,padding:"10px 12px",marginBottom:8,fontFamily:"monospace",fontSize:12}}>
                  <div style={{color:"var(--slate-400)"}}>input&nbsp;: <span style={{color:"var(--slate-200)"}}>{e.in}</span></div>
                  <div style={{color:"var(--slate-400)"}}>output: <span style={{color:"#86EFAC"}}>{e.out}</span></div>
                </div>
              ))}
            </div>
            {/* IDE */}
            <div style={{display:"flex",flexDirection:"column",background:"var(--slate-900)",minHeight:0}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",borderBottom:"1px solid #1E293B"}}>
                <div style={{display:"flex",gap:6}}>
                  {EXAM.coding.languages.map(l=>(
                    <button key={l} onClick={()=>onLang(l)} style={{padding:"5px 12px",borderRadius:6,border:0,fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer",background:lang===l?"var(--brand-500)":"#1E293B",color:lang===l?"var(--bg-surface)":"var(--slate-400)"}}>{l}</button>
                  ))}
                </div>
                <button onClick={runCode} disabled={running} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:7,border:0,background:"var(--success-500)",color:"var(--bg-surface)",fontWeight:600,fontSize:12,cursor:running?"wait":"pointer"}}>
              ...
              <div style={{flex:1,display:"flex",minHeight:0,position:"relative"}}>
                ...
                <textarea value={code} onChange={e=>setCode(e.target.value)} spellCheck={false}
                  ...
                  style={{flex:1,resize:"none",border:0,outline:"none",background:"var(--slate-900)",color:"var(--slate-200)",fontFamily:"monospace",fontSize:13,lineHeight:"21px",padding:"14px 16px",tabSize:2}}/>
              </div>
              {/* output */}
              <div style={{borderTop:"1px solid #1E293B",background:"#0B1220",padding:"12px 16px",maxHeight:170,overflowY:"auto"}}>
                ...
                {running && <div style={{fontFamily:"monospace",fontSize:12,color:"var(--slate-400)"}}>Running {EXAM.coding.tests.length} tests…</div>}
                {output && output.map((r,i)=>(
                  <div key={i} style={{fontFamily:"monospace",fontSize:12,display:"flex",gap:8,marginBottom:4,color:r.pass?"#86EFAC":"#FCA5A5"}}>
                    ...
                    <span><span style={{color:"var(--slate-400)"}}>{r.name}</span> → {r.got}</span>
                  </div>
                ))}
                {output && <div style={{fontFamily:"monospace",fontSize:12,color:"var(--slate-200)",marginTop:6,fontWeight:700}}>{output.filter(o=>o.pass).length}/{output.length} passed</div>}
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div style={{maxWidth:720,margin:"0 auto",padding:"32px 24px",animation:"v2fade 280ms"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
        <window.V2Av name={form.name||"R"} size={64} ring="var(--brand-100)"/>
        <div>
          ...
          <p style={{fontSize:13,color:"var(--slate-500)",marginTop:3}}>{form.role} · {form.loc}</p>
        </div>
      </div>
        ...
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:20,boxShadow:"0 1px 3px rgba(15,23,42,0.04)"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            ...
            <div style={{gridColumn:"1 / -1"}}>
              ...
              <input value={skillStr} onChange={e=>{setSkillStr(e.target.value);setSaved(false);}} style={{width:"100%",padding:"9px 12px",border:"1px solid var(--slate-300)",borderRadius:8,fontSize:13,fontFamily:"inherit",outline:"none"}}
                onFocus={e=>{e.target.style.borderColor="var(--brand-500)";e.target.style.boxShadow="0 0 0 3px rgba(91,79,233,0.18)";}}
                onBlur={e=>{e.target.style.borderColor="var(--slate-300)";e.target.style.boxShadow="none";}}/>
            </div>
            <div style={{gridColumn:"1 / -1"}}>
              <label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--slate-700)",marginBottom:5}}>About</label>
              <textarea value={form.about||""} onChange={e=>set("about",e.target.value)} rows={3} style={{width:"100%",padding:"10px 12px",border:"1px solid var(--slate-300)",borderRadius:8,fontSize:13,fontFamily:"inherit",lineHeight:1.55,outline:"none",resize:"vertical"}}
                onFocus={e=>{e.target.style.borderColor="var(--brand-500)";e.target.style.boxShadow="0 0 0 3px rgba(91,79,233,0.18)";}}
                onBlur={e=>{e.target.style.borderColor="var(--slate-300)";e.target.style.boxShadow="none";}}/>
            </div>
            <div style={{gridColumn:"1 / -1"}}>
              <label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--slate-700)",marginBottom:5}}>Resume</label>
              <div style={{border:"1px dashed var(--slate-300)",borderRadius:8,padding:"14px 12px",textAlign:"center",fontSize:13,color:"var(--slate-400)",cursor:"pointer"}}>
                <i data-lucide="upload-cloud" style={{width:15,height:15}}/> Replace resume (PDF / DOCX)
              </div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12,marginTop:18}}>
            <button onClick={save} style={{padding:"10px 18px",borderRadius:8,background:"var(--brand-500)",color:"var(--bg-surface)",border:0,fontWeight:600,fontSize:13,cursor:"pointer"}}>Save changes</button>
            {saved && <span style={{fontSize:13,color:"var(--success-600)",display:"inline-flex",alignItems:"center",gap:5}}><i data-lucide="check-circle-2" style={{width:14,height:14}}/>Saved</span>}
          </div>
        </div>
      )}
      {tab==="settings" && (
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:20}}>
            <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:8}}>Notifications</div>
            {[
              ...
              <div key={o.k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderTop:i?"1px solid var(--slate-100)":"0"}}>
                <div><div style={{fontSize:13,fontWeight:600,color:"var(--slate-900)"}}>{o.label}</div><div style={{fontSize:12,color:"var(--slate-500)",marginTop:1}}>{o.desc}</div></div>
                <window.V2Toggle on={settings[o.k]} onClick={()=>toggle(o.k)}/>
              </div>
            ))}
          </div>
          <div style={{background:"var(--bg-surface)",border:"1px solid var(--slate-200)",borderRadius:12,padding:20}}>
            <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--brand-500)",marginBottom:8}}>Security</div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0"}}>
              <div><div style={{fontSize:13,fontWeight:600,color:"var(--slate-900)"}}>Two-factor authentication</div><div style={{fontSize:12,color:"var(--slate-500)",marginTop:1}}>Extra security at sign-in</div></div>
              <window.V2Toggle on={settings.twoFactor} onClick={()=>toggle("twoFactor")}/>
            </div>
            <div style={{paddingTop:12,borderTop:"1px solid var(--slate-100)",display:"flex",gap:8}}>
              <button style={{padding:"8px 14px",borderRadius:8,border:"1px solid var(--slate-300)",background:"var(--bg-surface)",fontWeight:600,fontSize:13,color:"var(--slate-700)",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6}}><i data-lucide="key-round" style={{width:13,height:13}}/> Change password</button>
              <button style={{padding:"8px 14px",borderRadius:8,border:"1px solid var(--danger-100)",background:"var(--bg-surface)",fontWeight:600,fontSize:13,color:"var(--danger-700)",cursor:"pointer"}}>Delete my data</button>
            </div>
          </div>
        </div>
      )}
