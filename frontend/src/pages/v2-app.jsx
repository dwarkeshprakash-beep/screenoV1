          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {accounts.map(a=>(
              ...
              >
                ...
                <span style={{fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:6,background:"#1E293B",border:"1px solid #334155",color:"#94A3B8"}}>{a.label}</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div style={{padding:"24px 28px", maxWidth:1200}}>
      <div style={{marginBottom:28}}>
        <h1 style={{fontSize:26,fontWeight:700,color:"#0F172A",letterSpacing:"-0.02em"}}>Welcome back, Rahul!</h1>
        ...
        {[
          ...
          <div key={i} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:10,padding:"16px 20px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              ...
              <i data-lucide={s.icon} style={{width:16,height:16,color:"#94A3B8"}}/>
            </div>
          ...
          {CAND_UPCOMING.map((u,i)=>(
            <div key={u.id} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:10,padding:16,marginBottom:12,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                <div>
                  <p style={{fontSize:14,fontWeight:700,color:"#0F172A",margin:0}}>{u.role}</p>
                  <p style={{fontSize:12,color:"#64748B",margin:"3px 0 0"}}>{u.company}</p>
                </div>
                <span style={{fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:6,background:u.hasDate?"#EFEDFD":"#F1F5F9",color:u.hasDate?"#5B4FE9":"#64748B"}}>
                  {u.hasDate?"Scheduled":"Pending"}
                </span>
              </div>
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
                <span style={{fontSize:11,padding:"2px 8px",borderRadius:9999,background:"#EFEDFD",color:"#5B4FE9",fontWeight:600}}>{u.type}</span>
                <span style={{fontSize:11,padding:"2px 8px",borderRadius:9999,background:"#F1F5F9",color:"#475569",fontWeight:500}}>{u.duration}</span>
                <span style={{fontSize:11,padding:"2px 8px",borderRadius:9999,background:"#F1F5F9",color:"#475569",fontWeight:500}}>Attempt {u.attempt}</span>
              </div>
              {u.hasDate ? (
                ...
                <div style={{fontSize:12,color:"#D97706",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>
                  <i data-lucide="calendar" style={{width:12,height:12}}/>No date set — you can choose your own slot
                </div>
              )}
              <div style={{display:"flex",gap:8}}>
                {u.hasDate && <button onClick={()=>onStart(u.id,"c-device")} style={{flex:1,padding:"8px 14px",background:"#FFF",border:"1px solid #E2E8F0",borderRadius:7,fontSize:12,fontWeight:600,color:"#374151",cursor:"pointer"}}>Device Check</button>}
                {u.hasDate ? (
                  <button onClick={()=>onStart(u.id,"c-landing")} style={{flex:2,padding:"8px 14px",background:"#0F172A",border:0,borderRadius:7,fontSize:12,fontWeight:600,color:"#FFF",cursor:"pointer"}}>Start Interview →</button>
                ):(
                  <button onClick={()=>onSelfSchedule(u.id)} style={{flex:1,padding:"8px 14px",background:"#5B4FE9",border:0,borderRadius:7,fontSize:12,fontWeight:600,color:"#FFF",cursor:"pointer"}}>Choose my slot</button>
                )}
              </div>
            </div>
          ))}
        ...
        <div>
          ...
          <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:10,padding:16,marginBottom:20,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            {CAND_COMPLETED.length===0 ? (
              ...
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<CAND_COMPLETED.length-1?"1px solid #F1F5F9":"0"}}>
                <div>
                  <p style={{fontSize:13,fontWeight:600,color:"#0F172A",margin:0}}>{c.type}</p>
                  <p style={{fontSize:11,color:"#94A3B8",margin:"2px 0 0"}}>{c.date} · {c.feedback}</p>
                </div>
                <div style={{fontSize:20,fontWeight:700,color:c.score>=4?"#059669":"#D97706"}}>{c.score}</div>
              </div>
            ))}
          </div>
...
const SelfScheduleModal = ({open, onClose, interviewId, onConfirm}) => {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.55)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"v2fade 160ms"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#FFF",borderRadius:16,width:"100%",maxWidth:520,boxShadow:"0 24px 60px rgba(15,23,42,0.24)",animation:"v2scale 200ms cubic-bezier(0.2,0,0,1)",overflow:"hidden"}}>
        <div style={{padding:"22px 24px",borderBottom:"1px solid #F1F5F9",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <h2 style={{fontSize:16,fontWeight:700,color:"#0F172A",margin:0}}>Choose your interview slot</h2>
            <p style={{fontSize:12,color:"#64748B",margin:"4px 0 0"}}>Pick a date and time that works for you</p>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:0,color:"#94A3B8",cursor:"pointer"}}><i data-lucide="x" style={{width:18,height:18}}/></button>
        </div>
        <div style={{padding:"20px 24px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:20}}>
            {ds.map((d,i)=>(
              <button key={i} onClick={()=>{setSelDate(i);setSelTime(null);}} style={{padding:"10px 0",background:selDate===i?"#5B4FE9":"#F8FAFC",border:`1px solid ${selDate===i?"#5B4FE9":"#E2E8F0"}`,borderRadius:8,cursor:"pointer",textAlign:"center"}}>
                <div style={{fontSize:11,fontWeight:600,color:selDate===i?"rgba(255,255,255,0.7)":"#94A3B8"}}>{d.day}</div>
                <div style={{fontSize:13,fontWeight:700,color:selDate===i?"#FFF":"#0F172A",marginTop:2}}>{d.date.split(" ")[1]}</div>
                <div style={{fontSize:10,color:selDate===i?"rgba(255,255,255,0.7)":"#94A3B8"}}>{d.date.split(" ")[0]}</div>
              </button>
            ))}
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:24}}>
            {ds[selDate]?.slots.map((s,i)=>(
              <button key={i} onClick={()=>s.avail&&setSelTime(s.t)} disabled={!s.avail} style={{padding:"10px 0",background:selTime===s.t?"#5B4FE9":s.avail?"#FFF":"#F8FAFC",border:`1px solid ${selTime===s.t?"#5B4FE9":"#E2E8F0"}`,borderRadius:8,cursor:s.avail?"pointer":"not-allowed",opacity:s.avail?1:0.4,fontSize:13,fontWeight:600,color:selTime===s.t?"#FFF":s.avail?"#0F172A":"#94A3B8"}}>
                {s.t}
              </button>
            ))}
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={onClose} style={{flex:1,padding:"10px 16px",border:"1px solid #E2E8F0",background:"#FFF",color:"#0F172A",borderRadius:8,fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
            <button onClick={()=>{onConfirm(interviewId,ds[selDate]?.date,selTime);onClose();}} disabled={!selTime} style={{flex:2,padding:"10px 16px",background:selTime?"#5B4FE9":"#CBD5E1",border:0,borderRadius:8,fontWeight:600,fontSize:13,color:"#FFF",cursor:selTime?"pointer":"not-allowed"}}>
...
const CompareScreen = ({onBack}) => {
  ...
  const winColor = (v1,v2) => v1>v2?"#5B4FE9":v2>v1?"#059669":"#94A3B8";
  return (
    <div style={{padding:"24px 28px"}}>
      <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:6,background:"transparent",border:0,color:"#5B4FE9",fontWeight:500,fontSize:13,cursor:"pointer",marginBottom:24}}>
        <i data-lucide="arrow-left" style={{width:16,height:16}}/>Back
      </button>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
        <div>
          <h1 style={{fontSize:24,fontWeight:700,color:"#0F172A",letterSpacing:"-0.02em",margin:0}}>Compare Candidates</h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"240px 1fr 240px",gap:24,marginBottom:28,alignItems:"start"}}>
        {[c1,c2].map((c,i)=>(
          <div key={i} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:12,padding:18,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <V2Av name={c.name} size={40}/>
              <span style={{fontSize:11,fontWeight:700,padding:"3px 8px",borderRadius:6,background:c.decision==="PASS"?"#ECFDF5":"#FFFBEB",color:c.decision==="PASS"?"#059669":"#D97706"}}>{c.decision}</span>
            </div>
            <p style={{fontSize:14,fontWeight:700,color:"#0F172A",margin:"0 0 2px"}}>{c.name}</p>
      ...
      <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:12,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        <div style={{display:"grid",gridTemplateColumns:"240px 1fr 1fr",background:"#F8FAFC",padding:"10px 18px",borderBottom:"1px solid #E2E8F0"}}>
          <span style={{fontSize:11,fontWeight:700,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.06em"}}>Metric</span>
          <span style={{fontSize:12,fontWeight:700,color:"#0F172A",padding:"0 16px"}}>{c1.name}</span>
          <span style={{fontSize:12,fontWeight:700,color:"#0F172A",padding:"0 16px"}}>{c2.name}</span>
        </div>
          ...
          return (
            <div key={m.key} style={{display:"grid",gridTemplateColumns:"240px 1fr 1fr",padding:"12px 18px",borderBottom:i<COMPARE_METRICS.length-1?"1px solid #F9FAFB":"0",alignItems:"center"}}
              ...
              <div style={{padding:"0 16px",display:"flex",alignItems:"center",gap:10}}>
                ...
                <span style={{fontSize:13,fontWeight:700,minWidth:36,color:v1>v2?"#5B4FE9":"#374151"}}>{fmtV(v1,m.max)}</span>
              </div>
              <div style={{padding:"0 16px",display:"flex",alignItems:"center",gap:10}}>
                {bar(v2,m.max,winColor(v2,v1))}
                <span style={{fontSize:13,fontWeight:700,minWidth:36,color:v2>v1?"#059669":"#374151"}}>{fmtV(v2,m.max)}</span>
              </div>
            </div>
          );
        })}
...
const TemplatesScreen = ({onBack, onUse}) => {
  ...
  return (
    <div style={{padding:"24px 28px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
        <div>
          <h1 style={{fontSize:24,fontWeight:700,color:"#0F172A",letterSpacing:"-0.02em",margin:0}}>Interview Templates</h1>
          <p style={{fontSize:13,color:"#64748B",marginTop:4}}>Pre-configured settings you can reuse when scheduling</p>
        </div>
        <button onClick={()=>setCustom(true)} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 16px",background:"#5B4FE9",color:"#FFF",border:0,borderRadius:8,fontWeight:600,fontSize:13,cursor:"pointer"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:16,marginBottom:28}}>
        {myTemplates.map(t=>(
          <div key={t.id} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:12,padding:20,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <h3 style={{fontSize:15,fontWeight:700,color:"#0F172A",margin:0}}>{t.name}</h3>
              <span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:6,background:"#EFEDFD",color:"#5B4FE9"}}>{t.type}</span>
            </div>
            ...
            <p style={{fontSize:11,color:"#94A3B8",margin:"0 0 14px",fontStyle:"italic"}}>"{t.prompt}"</p>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>onUse(t)} style={{flex:1,padding:"8px 12px",background:"#5B4FE9",color:"#FFF",border:0,borderRadius:7,fontSize:12,fontWeight:600,cursor:"pointer"}}>Use Template</button>
              <button style={{padding:"8px 12px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:7,fontSize:12,fontWeight:600,color:"#374151",cursor:"pointer"}}>Edit</button>
            </div>
          </div>
        ))}
      </div>
      {custom && (
        <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:12,padding:20}}>
          <h3 style={{fontSize:15,fontWeight:700,color:"#0F172A",margin:"0 0 16px"}}>Create Custom Template</h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            <div><label style={{fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:6}}>Template Name</label>
            <input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="e.g. React Frontend Standard" style={{width:"100%",padding:"8px 12px",border:"1px solid #CBD5E1",borderRadius:7,fontSize:13,fontFamily:"inherit",outline:"none"}}/></div>
            <div><label style={{fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:6}}>Attempts</label>
            <select value={newAttempts} onChange={e=>setNewAttempts(+e.target.value)} style={{width:"100%",padding:"8px 12px",border:"1px solid #CBD5E1",borderRadius:7,fontSize:13,fontFamily:"inherit",outline:"none",background:"#FFF"}}>
          ...
          <div style={{marginTop:12}}><label style={{fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:6}}>Description / Prompt Focus</label>
          <textarea rows={2} value={newDesc} onChange={e=>setNewDesc(e.target.value)} placeholder="Describe focus areas, tech stack, or special requirements…" style={{width:"100%",padding:"8px 12px",border:"1px solid #CBD5E1",borderRadius:7,fontSize:13,fontFamily:"inherit",outline:"none",resize:"none"}}/></div>
          <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"flex-end"}}>
            <button onClick={()=>setCustom(false)} style={{padding:"8px 16px",background:"#FFF",border:"1px solid #E2E8F0",borderRadius:7,fontSize:13,fontWeight:600,color:"#374151",cursor:"pointer"}}>Cancel</button>
            <button onClick={()=>{if(newName){setMyTemplates(p=>[...p,{id:`t${p.length+1}`,name:newName,desc:newDesc||"Custom template",attempts:newAttempts,type:"AI Voice",reportAfter:"all",prompt:newDesc}]);setCustom(false);setNewName("");setNewDesc("");}}} style={{padding:"8px 16px",background:"#5B4FE9",border:0,borderRadius:7,fontSize:13,fontWeight:600,color:"#FFF",cursor:"pointer"}}>Save Template</button>
          </div>
        </div>
      )}
    </div>
...
const ResumeAnalyzerScreen = ({onBack}) => {
  ...
  const verdict = (s) => s>=85 ? {label:"Strong match — likely passes ATS", c:"#047857", bg:"#ECFDF5", bd:"#A7F3D0", icon:"check-circle-2", note:"Strongly aligned. Most ATS filters will pass this resume through."}
    : s>=65 ? {label:"Good shape — passes most filters", c:"#047857", bg:"#ECFDF5", bd:"#A7F3D0", icon:"thumbs-up", note:"In good shape. Close the remaining gaps for competitive, high-volume roles."}
    : s>=50 ? {label:"Borderline — may be filtered in large pools", c:"#B45309", bg:"#FFFBEB", bd:"#FEF3C7", icon:"alert-triangle", note:"Might pass in a small applicant pool, but likely filtered when volume is high."}
    : {label:"Major mismatch — likely rejected", c:"#B53618", bg:"#FEF2F2", bd:"#FECACA", icon:"circle-x", note:"Significant skills gap for this JD. Add the missing required skills first."};

  const chip = (label, tone) => (
    <span key={label} style={{fontSize:12,fontWeight:600,padding:"3px 10px",borderRadius:9999,
      background:tone==="ok"?"#ECFDF5":tone==="miss"?"#FEF2F2":"#F1F5F9",
      color:tone==="ok"?"#047857":tone==="miss"?"#B53618":"#475569"}}>{label}</span>
  );
  ...
  return (
    <div style={{padding:"24px 28px",maxWidth:940}}>
      ...
      <h1 style={{fontSize:24,fontWeight:700,color:"#0F172A",letterSpacing:"-0.02em",margin:"0 0 4px"}}>Resume Analyzer</h1>
            ...
            {[{t:"Job description",v:jd,set:setJd,ph:"Paste the full JD here…",sample:JD_SAMPLE},
              ...
              <div key={i} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:12,padding:16,boxShadow:"0 1px 3px rgba(15,23,42,0.04)"}}>
                ...
                <textarea value={c.v} onChange={e=>c.set(e.target.value)} placeholder={c.ph} rows={12}
                  style={{width:"100%",padding:12,border:"1px solid #CBD5E1",borderRadius:8,fontSize:13,fontFamily:"inherit",lineHeight:1.55,outline:"none",resize:"vertical"}}
                  onFocus={e=>{e.target.style.borderColor="#5B4FE9";e.target.style.boxShadow="0 0 0 3px rgba(91,79,233,0.18)";}}
                  onBlur={e=>{e.target.style.borderColor="#CBD5E1";e.target.style.boxShadow="none";}}/>
              </div>
            ))}
          </div>
          <button onClick={analyze} disabled={phase==="analyzing"||!jd.trim()||!resume.trim()} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"11px 20px",background:(!jd.trim()||!resume.trim())?"#CBD5E1":"#5B4FE9",color:"#FFF",border:0,borderRadius:8,fontWeight:600,fontSize:13,cursor:(!jd.trim()||!resume.trim())?"not-allowed":"pointer"}}>
      ...
      {phase==="results" && res && (()=>{
        ...
        return (
          <div>
            <button onClick={()=>setPhase("input")} style={{fontSize:12,color:"#5B4FE9",fontWeight:600,background:"transparent",border:0,cursor:"pointer",marginBottom:14,display:"inline-flex",alignItems:"center",gap:5}}><i data-lucide="rotate-ccw" style={{width:13,height:13}}/>Edit inputs &amp; re-scan</button>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:20}}>
              <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:10,padding:16,textAlign:"center"}}>
                <p style={{fontSize:26,fontWeight:700,color:"#047857",margin:0}}>{res.mH.length+res.mS.length}</p>
              ...
              <div style={{background:"#FFFBEB",border:"1px solid #FCD34D",borderRadius:10,padding:16,textAlign:"center"}}>
                <p style={{fontSize:26,fontWeight:700,color:"#B45309",margin:0}}>{res.missH.length}</p>
                <p style={{fontSize:12,color:"#92400E",marginTop:4}}>Hard skills to focus on</p>
              </div>
            </div>

            {res.missH.length>0 && (
              <div style={{background:"#FFFBEB",border:"1px solid #FCD34D",borderRadius:10,padding:"14px 16px",marginBottom:20}}>
                <p style={{fontSize:13,color:"#92400E",fontWeight:700,margin:"0 0 6px",display:"flex",alignItems:"center",gap:6}}><i data-lucide="target" style={{width:14,height:14}}/>Focus the interview here</p>
            ...
            <div style={{display:"flex",gap:4,marginBottom:14,borderBottom:"1px solid #E2E8F0"}}>
              {[["hard","Hard skills"],["soft","Soft skills"],["search","Searchability"]].map(([id,l])=>(
                <button key={id} onClick={()=>setTab(id)} style={{padding:"10px 16px",border:0,background:"transparent",fontSize:13,fontWeight:600,color:tab===id?"#5B4FE9":"#94A3B8",borderBottom:tab===id?"2px solid #5B4FE9":"2px solid transparent",cursor:"pointer"}}>{l}</button>
              ))}
            </div>

            {tab==="hard" && (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div><div style={{fontSize:12,fontWeight:600,color:"#047857",marginBottom:8}}>Matched ({res.mH.length})</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{res.mH.length?res.mH.map(s=>chip(s,"ok")):<span style={{fontSize:13,color:"#94A3B8"}}>None matched</span>}</div></div>
                <div><div style={{fontSize:12,fontWeight:600,color:"#B53618",marginBottom:8}}>Missing ({res.missH.length})</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{res.missH.length?res.missH.map(s=>chip(s,"miss")):<span style={{fontSize:13,color:"#94A3B8"}}>Nothing missing — great</span>}</div></div>
              </div>
            )}
            {tab==="soft" && (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div><div style={{fontSize:12,fontWeight:600,color:"#047857",marginBottom:8}}>Matched ({res.mS.length})</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{res.mS.length?res.mS.map(s=>chip(s,"ok")):<span style={{fontSize:13,color:"#94A3B8"}}>None matched</span>}</div></div>
                <div><div style={{fontSize:12,fontWeight:600,color:"#B53618",marginBottom:8}}>Missing ({res.missS.length})</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{res.missS.length?res.missS.map(s=>chip(s,"miss")):<span style={{fontSize:13,color:"#94A3B8"}}>Nothing missing</span>}</div></div>
              </div>
            )}
            {tab==="search" && (
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {res.search.map((s,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:s.ok?"#ECFDF5":"#FEF2F2",border:"1px solid "+(s.ok?"#A7F3D0":"#FECACA"),borderRadius:8}}>
                    <i data-lucide={s.ok?"check-circle-2":"circle-x"} style={{width:16,height:16,color:s.ok?"#059669":"#DC2626"}}/>
                    <span style={{fontSize:13,color:"#0F172A"}}>{s.label}</span>
                  </div>
                ))}
                <p style={{fontSize:12,color:"#94A3B8",marginTop:4}}>ATS systems must be able to parse contact details and standard sections to rank a resume.</p>
              </div>
            )}
          </div>
        );
...
const PlaceholderPage = ({title,icon,body}) => {
  React.useEffect(()=>{window.lucide?.createIcons();},[]);
  return (
    <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:12,padding:"80px 32px",textAlign:"center",color:"#6B7280"}}>
      <i data-lucide={icon||"layers"} style={{width:36,height:36,color:"#94A3B8",margin:"0 auto 14px"}}/>
      <div style={{fontWeight:600,fontSize:18,color:"#0F172A",marginBottom:6}}>{title}</div>
...
const CandidateExamPlaceholder = ({onNext}) => {
  ...
  return (
    <div style={{maxWidth:680,margin:"0 auto",padding:"48px 24px",textAlign:"center"}}>
      <div style={{width:56,height:56,borderRadius:14,background:"#EFF6FF",color:"#2563EB",display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:16}}>
        <i data-lucide="code-2" style={{width:26,height:26}}/>
      </div>
      <h1 style={{fontSize:26,fontWeight:700,color:"#0F172A",margin:"0 0 8px",letterSpacing:"-0.02em"}}>Coding exam</h1>
      <p style={{fontSize:14,color:"#6B7280",marginBottom:24}}>45 minutes · 10 questions</p>
      <button onClick={onNext} style={{padding:"12px 28px",borderRadius:10,background:"#5B4FE9",color:"#FFF",border:0,fontWeight:600,fontSize:14,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:8}}>
    ...
    <div style={{minHeight:"calc(100vh - 132px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
      <div style={{maxWidth:560,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:22}}>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:"#5B4FE9"}}>Tech Round 1 · with Anand Rao</div>
          <h1 style={{fontFamily:"var(--font-display,'Inter')",fontSize:26,fontWeight:700,color:"#0F172A",letterSpacing:"-0.02em",margin:"8px 0 4px"}}>You're ready to join</h1>
          <p style={{fontSize:14,color:"#6B7280",margin:0}}>Senior .NET Developer · 60 minutes · today 2:00 PM IST</p>
        </div>
        {/* preview */}
        <div style={{position:"relative",height:220,borderRadius:14,overflow:"hidden",background:camOn?"linear-gradient(135deg,#475569,#1E293B)":"#0F172A",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16}}>
          ...
          <div style={{position:"absolute",bottom:12,left:"50%",transform:"translateX(-50%)",display:"flex",gap:10}}>
            <button onClick={()=>setMicOn(m=>!m)} style={{width:44,height:44,borderRadius:9999,border:0,cursor:"pointer",background:micOn?"rgba(255,255,255,0.15)":"#EF4444",color:"#FFF",display:"inline-flex",alignItems:"center",justifyContent:"center"}}><i data-lucide={micOn?"mic":"mic-off"} style={{width:18,height:18}}/></button>
            <button onClick={()=>setCamOn(c=>!c)} style={{width:44,height:44,borderRadius:9999,border:0,cursor:"pointer",background:camOn?"rgba(255,255,255,0.15)":"#EF4444",color:"#FFF",display:"inline-flex",alignItems:"center",justifyContent:"center"}}><i data-lucide={camOn?"video":"video-off"} style={{width:18,height:18}}/></button>
          </div>
        </div>
        {/* provider */}
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {providers.map(o=>(
            <button key={o.id} onClick={()=>setProvider(o.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"12px 8px",borderRadius:10,fontFamily:"inherit",
              border:`1px solid ${provider===o.id?"#5B4FE9":"#E2E8F0"}`,background:provider===o.id?"#EFEDFD":"#FFF",color:provider===o.id?"#3A31A3":"#374151",fontWeight:600,fontSize:12,cursor:"pointer"}}>
              <i data-lucide={o.icon} style={{width:18,height:18}}/>{o.label}
            </button>
          ))}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,fontSize:12,color:"#6B7280",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:8,padding:"9px 12px",marginBottom:16}}>
          <i data-lucide="link" style={{width:13,height:13,color:"#94A3B8"}}/>{p.sub}
        </div>
        <button onClick={onJoin} style={{width:"100%",padding:"14px",borderRadius:12,background:"#059669",color:"#FFF",border:0,fontSize:15,fontWeight:600,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 8px 20px rgba(5,150,105,0.25)"}}>
          <i data-lucide={p.icon} style={{width:18,height:18}}/> Join on {p.label}
        </button>
        <p style={{fontSize:12,color:"#94A3B8",textAlign:"center",marginTop:12}}>Your interviewer will admit you when they're ready.</p>
      </div>
    </div>
  );
};
...
function V2App() {
  ...
  if(role==="candidate" && (page==="c-dashboard" || page==="c-profile")) return (
    <>
      <window.V2RoleBar role={role} onRole={handleRoleChange} onLogout={handleLogout}/>
      <div style={{background:"#FFF",borderBottom:"1px solid #E2E8F0",padding:"0 24px",height:54,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:38,zIndex:99}}>
        <h1 style={{fontSize:16,fontWeight:700,color:"#0F172A",cursor:"pointer"}} onClick={()=>setPage("c-dashboard")}>My Interviews</h1>
        <button onClick={()=>setPage("c-profile")} style={{display:"inline-flex",alignItems:"center",gap:8,background:page==="c-profile"?"#F1F5F9":"transparent",border:0,borderRadius:8,padding:"5px 8px",cursor:"pointer"}}>
          <window.V2Av name={store?.get?.().candidate?.name||"Rahul Sharma"} size={30}/>
          <span style={{fontSize:13,color:"#374151",fontWeight:500}}>{store?.get?.().candidate?.name||"Rahul Sharma"}</span>
          <i data-lucide="chevron-down" style={{width:13,height:13,color:"#94A3B8"}}/>
        </button>
      </div>
      <div style={{background:"#F8FAFC",minHeight:"calc(100vh - 92px)"}}>
        {page==="c-dashboard" && (
