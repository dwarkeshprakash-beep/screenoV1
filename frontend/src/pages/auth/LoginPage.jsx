...
const DEMO_ACCOUNTS = [
  ...
  { role: 'candidate',   name: 'Raj Rathod',        sub: 'Candidate · Software Engineer',        initials: 'RR', bgColor: '#A7F3D0', fgColor: '#065F46', badgeBg: '#064E3B', badgeFg: '#6EE7B7', badge: 'candidate',   email: 'raj.rathod@gmail.com',                   password: 'psspl@1234' },
]
...
function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100svh', background: '#0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem' }}>
      <div style={{ width: '100%', maxWidth: '28rem' }}>

        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.625rem', marginBottom: '1rem' }}>
            <div style={{ width: '2.75rem', height: '2.75rem', borderRadius: '0.75rem', background: 'linear-gradient(135deg,#5B4FE9,#4A3FCE)', position: 'relative' }}>
              <div style={{ position: 'absolute', left: '25%', top: '32%', width: '50%', height: '10%', background: '#FFF', borderRadius: '0.125rem', opacity: 0.95 }} />
              <div style={{ position: 'absolute', left: '25%', top: '56%', width: '50%', height: '10%', background: '#FFF', borderRadius: '0.125rem', opacity: 0.6 }} />
            </div>
            <span style={{ fontSize: '1.75rem', fontWeight: 700, color: '#FFF', letterSpacing: '-0.025em' }}>Screeno</span>
          </div>
        ...
        <div style={{ background: '#1E293B', border: '1px solid #334155', borderRadius: '1rem', padding: '1.75rem' }}>
          <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: '#94A3B8', letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: '1.125rem' }}>Demo Accounts</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div>
              <label style={{ color: '#94A3B8', fontSize: '0.8125rem', fontWeight: 500, display: 'block', marginBottom: '0.375rem' }}>Email</label>
              <input type="email" placeholder="you@company.com" value={email} onChange={e => setEmail(e.target.value)} required style={inputStyle} />
            </div>
            <div>
              <label style={{ color: '#94A3B8', fontSize: '0.8125rem', fontWeight: 500, display: 'block', marginBottom: '0.375rem' }}>Password</label>
              <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required style={inputStyle} />
            </div>

            {error && <p style={{ fontSize: '0.8125rem', color: '#EF4444' }}>{error}</p>}

            <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.75rem', background: '#5B4FE9', color: '#fff', border: 'none', borderRadius: '0.5rem', fontSize: '0.875rem', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, marginTop: '0.25rem' }}>
...
function DemoButton({ account, onSelect }) {
  ...
  return (
    <button
      ...
      style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', padding: '0.875rem 1rem', background: '#0F172A', border: `1px solid ${hovered ? '#5B4FE9' : '#334155'}`, borderRadius: '0.625rem', cursor: 'pointer', textAlign: 'left', transition: 'border-color 150ms', width: '100%' }}
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '0.875rem', fontWeight: 600, color: '#FFF' }}>{account.name}</div>
