...
const DeviceCheckScreen = ({ onContinue }) => {
  ...
  return (
    <window.Reveal style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: 16, padding: 32, boxShadow: "0 8px 28px rgba(15,23,42,0.06), 0 2px 6px rgba(15,23,42,0.04)", maxWidth: 520, margin: "0 auto" }}>
      <h1 style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 700, color: "#0F172A", letterSpacing: "-0.02em", margin: 0 }}>
        Let's check your device
      </h1>
      <p style={{ fontSize: 14, color: "#6B7280", margin: "6px 0 24px" }}>This takes about 30 seconds.</p>
      ...
      <div style={{ marginTop: 24 }}>
        {allPassed && (
          <window.Reveal style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: 12, background: "#ECFDF5", border: "1px solid #A7F3D0", borderRadius: 10, color: "#047857", fontSize: 13, fontWeight: 600, marginBottom: 14 }}>
        ...
        <button
          ...
          style={{
            ...
            background: allPassed ? "#5B4FE9" : "#E2E8F0",
            color: allPassed ? "#FFF" : "#94A3B8",
          ...
          onMouseEnter={e => { if (allPassed) e.currentTarget.style.background = "#4A3FCE"; }}
          onMouseLeave={e => { if (allPassed) e.currentTarget.style.background = "#5B4FE9"; }}
        >
...
const CheckRow = ({ check, status, index, extra, level, onSpeakerTest, speakerClicked }) => {
  ...
  const iconBg = tone === "success" ? "#ECFDF5" : tone === "danger" ? "#FFEDE6" : "#F1F5F9";
  const iconFg = tone === "success" ? "#047857" : tone === "danger" ? "#B53618" : "#94A3B8";
  return (
    <window.Reveal delay={index * 80} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", border: "1px solid #E2E8F0", borderRadius: 12, background: status === "passed" ? "#FAFAFE" : "#FFF", transition: "all 240ms" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#0F172A" }}>{check.label}</div>
        <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2, minHeight: 16 }}>
          ...
          {status === "failed" && <span style={{ color: "#B53618" }}>Failed — check permissions</span>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        {extra === "camera" && status === "passed" && (
          <window.Pop>
            <div style={{ width: 40, height: 40, borderRadius: 9999, overflow: "hidden", background: "linear-gradient(135deg, #475569, #1E293B)", display: "flex", alignItems: "center", justifyContent: "center", color: "#FFF", fontSize: 11, fontWeight: 600, boxShadow: "0 2px 6px rgba(15,23,42,0.16)", border: "2px solid #FFF" }}>
        ...
        {extra === "network-speed" && status === "passed" && (
          <window.Pop>
            <span style={{ fontSize: 13, fontFamily: "var(--font-mono)", fontWeight: 600, color: "#0F172A" }}>18 Mbps</span>
          </window.Pop>
        )}
        ...
        {status === "checking" && (
          <window.Icon name="loader-2" size={20} color="#94A3B8" style={{ animation: "spin 1s linear infinite" }} />
        )}
        {status === "failed" && <window.Icon name="x-circle" size={20} color="#FF5C35" />}
      </div>
...
const ConsentScreen = ({ onAgree, onDecline }) => {
  ...
  return (
    <window.Reveal style={{ maxWidth: 600, margin: "0 auto" }}>
      <h1 style={{ fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 700, color: "#0F172A", letterSpacing: "-0.025em", margin: "0 0 6px" }}>
        Before you begin
      </h1>
      ...
      <div style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: 14, padding: 24, marginBottom: 18, boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
        {items.map((it, i) => (
          <window.Reveal key={it.title} delay={i * 60} style={{ display: "flex", gap: 14, padding: "14px 0", borderTop: i === 0 ? 0 : "1px solid #F1F5F9", alignItems: "flex-start" }}>
            <div style={{ width: 36, height: 36, borderRadius: 9, background: "#EFEDFD", color: "#5B4FE9", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 10, padding: 14, background: agreed ? "#F3F0FF" : "#F8FAFC", border: `1px solid ${agreed ? "#5B4FE9" : "#E2E8F0"}`, borderRadius: 10, cursor: "pointer", marginBottom: 14, fontSize: 13, color: "#0F172A", fontWeight: 500, transition: "all 160ms cubic-bezier(0.2, 0, 0, 1)" }}>
        <span style={{
          width: 20, height: 20, borderRadius: 5,
          background: agreed ? "#5B4FE9" : "#FFF",
          border: `1.5px solid ${agreed ? "#5B4FE9" : "#CBD5E1"}`,
          display: "inline-flex", alignItems: "center", justifyContent: "center",
          flexShrink: 0, transition: "all 160ms",
        }}>
          {agreed && <window.Icon name="check" size={14} color="#FFF" />}
        </span>
      ...
      <button
        ...
        style={{
          width: "100%", padding: "13px 22px",
          background: agreed ? "#5B4FE9" : "#E2E8F0",
        ...
        onMouseEnter={e => { if (agreed) e.currentTarget.style.background = "#4A3FCE"; }}
        onMouseLeave={e => { if (agreed) e.currentTarget.style.background = "#5B4FE9"; }}
      >
        Start interview <window.Icon name="arrow-right" size={14} />
      </button>

      <div style={{ textAlign: "center", marginTop: 14 }}>
        <button onClick={onDecline} style={{ background: "transparent", border: 0, padding: 6, color: "#94A3B8", fontSize: 13, fontWeight: 500, cursor: "pointer", textDecoration: "underline", textDecorationColor: "transparent", transition: "all 120ms" }}
          onMouseEnter={e => { e.currentTarget.style.color = "#6B7280"; e.currentTarget.style.textDecorationColor = "#6B7280"; }}
          onMouseLeave={e => { e.currentTarget.style.color = "#94A3B8"; e.currentTarget.style.textDecorationColor = "transparent"; }}
        >
          I do not consent
        </button>
      </div>

      <p style={{ fontSize: 11, color: "#94A3B8", textAlign: "center", margin: "18px 0 0", lineHeight: 1.6 }}>
        You can request data deletion at any time by emailing <a style={{ color: "#5B4FE9" }}>privacy@screeno.io</a>
      </p>
    </window.Reveal>
  );
};
...
const CompletionScreen = ({ onClose, candidate = { name: "Rahul Sharma", first: "Rahul", role: "Senior .NET Developer", company: "TechCorp India", email: "rahul.sharma@gmail.com" } }) => {
  ...
  return (
    <div style={{ textAlign: "center", maxWidth: 640, margin: "0 auto" }}>
      <window.Pop>
        <div style={{ width: 80, height: 80, borderRadius: 9999, background: "#D1FAE5", display: "inline-flex", alignItems: "center", justifyContent: "center", boxShadow: "0 12px 28px rgba(5,150,105,0.18), 0 0 0 8px rgba(16,185,129,0.08)" }}>
      ...
      <window.Reveal delay={300}>
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: 30, fontWeight: 700, color: "#0F172A", letterSpacing: "-0.025em", margin: "24px 0 8px" }}>
          You're all done!
        </h1>
        <p style={{ fontSize: 15, color: "#374151", margin: 0, lineHeight: 1.6, maxWidth: 480, marginLeft: "auto", marginRight: "auto" }}>
          Thanks for completing the interview for <strong style={{ color: "#0F172A" }}>{candidate.role}</strong> at <strong style={{ color: "#0F172A" }}>{candidate.company}</strong>.
        </p>
      </window.Reveal>
      ...
      <div style={{ textAlign: "left" }}>
        <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#5B4FE9", textAlign: "center", marginBottom: 18 }}>
          What happens next
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {steps.map((s, i) => (
            <window.Reveal key={s.title} delay={420 + i * 100} style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: 12, padding: 20, boxShadow: "0 1px 3px rgba(15,23,42,0.04)", textAlign: "left" }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: "#0F172A", marginBottom: 4 }}>{s.title}</div>
              <div style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.55 }}>{s.body}</div>
            </window.Reveal>
          ))}
        </div>
      </div>
      ...
      <button
        onClick={onClose}
        style={{ marginTop: 32, padding: "11px 20px", background: "transparent", border: "1px solid #CBD5E1", borderRadius: 10, fontSize: 14, fontWeight: 600, color: "#374151", cursor: "pointer", transition: "all 120ms" }}
