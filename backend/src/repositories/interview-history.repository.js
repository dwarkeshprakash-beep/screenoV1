...
async function getPlatformHistory(managerId, userId = null) {
  ...
  return db.query(
    `SELECT
       ...
       COALESCE(cmr.profile_name, ct.requirements, ma.subject_name, i.subject_name) AS role_name,
       COALESCE(ct.client_name, ma.subject_name, i.subject_name, 'General interview') AS context_title,
       i.type,
       i.interview_mode,
       i.difficulty,
       i.status,
...
// Same two record kinds as getByManager, scoped to the whole company instead of one
// manager - the View All tier of Team's interview history.
async function getPlatformHistoryForCompany(companyId, userId = null) {
  const userFilter = userId == null ? '' : 'AND i.internal_user_id = @userId'
  return db.query(
    `SELECT
       i.id,
     ...
     WHERE mgr.company_id = @companyId
       ${userFilter}
     ORDER BY COALESCE(i.scheduled_at, i.created) DESC`,
    { companyId, ...(userId == null ? {} : { userId }) }
  )
}

async function getClientRoundHistoryForCompany(companyId, userId = null) {
  const userFilter = userId == null ? '' : 'AND cteam.user_id = @userId'
  return db.query(
    `SELECT
       cr.id,
     ...
     JOIN client_teams cteam ON cteam.id = cr.client_team_id
     JOIN client_templates ct ON ct.id = cteam.mandate_id
     JOIN users mgr ON mgr.id = ct.manager_id
     JOIN users u ON u.id = cteam.user_id
     LEFT JOIN client_mandate_requirements cmr ON cmr.id = cteam.requirement_id
     WHERE mgr.company_id = @companyId
       ${userFilter}
...
async function getByCompany(companyId, userId = null) {
  const [platform, clientRounds] = await Promise.all([
    getPlatformHistoryForCompany(companyId, userId),
    getClientRoundHistoryForCompany(companyId, userId),
  ])
  return [...platform, ...clientRounds].sort((a, b) => {
    const aDate = new Date(a.scheduled_at || a.created || 0).getTime()
    const bDate = new Date(b.scheduled_at || b.created || 0).getTime()
...
module.exports = { getByManager, getByCompany }
